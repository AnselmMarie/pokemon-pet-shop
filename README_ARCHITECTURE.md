...
| **Separated Libraries** | Codebase is strictly organized into distinct libraries (`features`, `types`, `services`, `ui/primitives`, `ui/components`, `ui/ds-tokens`, `utils`). This design enforces a **strong separation of concerns**, preventing circular dependencies and allowing specific libraries (e.g., `types`) to be imported into both web and API projects without conflict. |
...
| **Layered Design System Pattern** | The UI library follows a **layered architecture** with clear separation between primitives and components. **Primitives** (`libs/ui/primitives`) are basic building blocks (Box, Typography, Image, Container, Card, Link) with minimal dependencies. **Components** (`libs/ui/components`) are composed UI elements that build on primitives (Button, Input, Icon, Modal, etc.). **Design Tokens** (`libs/ui/ds-tokens`) provide the foundational styling variables. This pattern enforces dependency rules via ESLint module boundaries, ensuring primitives remain independent while components can depend on primitives and tokens. |
